#include "planck.h"
#include "config.h"
#include "mousekey.h"
#include "action_layer.h"
#ifdef AUDIO_ENABLE
  #include "audio.h"
#endif
#include "eeconfig.h"
#include "keymap_extras/keymap_swedish.h"
#include "sv.h"
#include "i3.h"


extern keymap_config_t keymap_config;

#define _BASE 0
#define _MOVE 1
#define _NUMPAD 2
#define _PARENS 3
#define _SIGNS 4
#define _MOUSE 5
#define _CONTROLS 6
#define _FUNC 7
#define _I3WS 8
#define _I3MV 9

#define ENDASH LALT(KC_MINS)
#define POUND ALGR(KC_3)
#define EURO ALGR(KC_E)
#define H(X) LALT(LCTL(X))

// Fillers to make layering more clear
#define _______ KC_TRNS
#define XXXXXXX KC_NO
#define C(n)    LCTL(n)
#define CA(n)   LCTL(LALT(n))
#define CAS(n)  LCTL(LALT(LSFT(n)))
#define CS(n)   LCTL(LSFT(n))
#define AS(n)   LALT(LSFT(n))
#define SLACKUP AS(KC_UP)
#define SLACKDN AS(KC_DOWN)

#define THUMB_ROW { KC_DEL, KC_LGUI, KC_LALT, ALGR_T(KC_INS), CTL_T(KC_BSPC), GUI_T(KC_DEL), ALT_T(KC_SPC), SFT_T(KC_ENT), KC_RALT, KC_RCTL, KC_ENT, KC_ENT}
/*
 * |------+-------+------+------------+----------+-----------+----------+----------+------+------+------+------|
 * | Del  | Gui   |  Alt | Altgr(Ins) |Ctrl(Bspc)| Gui(Del)  | Alt(Spc) | Sft(Ent) |  Alt | Ctrl | Ent  | Ent  |
 * `-----------------------------------------------------------------------------------------------------------'*/

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {

/* BASE
 * ,-----------------------------------------------------------------------------------.
 * | Esc  |   Ä  | Q/num|,/i3ws|./i3mv|  Y   | F    |   G  |   L  |   H  |   M  | Bksp |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |Tab/ms| O/mv |   I  |   E  |   A  |  U   | D    |   S  |   T  |   R  |   N  |  P   |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |  /   |Ö/fun |J/cont|K/sign| Å/par|  Z   | B    |   C  |   X  |   V  |   W  |  -_  |
 * `-----------------------------------------------------------------------------------' */
[_BASE] = {
  { KC_ESC,            NO_AE,             LT(_NUMPAD,KC_Q),  LT(_I3WS, KC_COMM), LT(_I3MV, KC_DOT), KC_Y, KC_F, KC_G, KC_L, KC_H, KC_M, KC_BSPC },
  { LT(_MOUSE,KC_TAB), LT(_MOVE,KC_O),    KC_I,              KC_E,              KC_A,               KC_U, KC_D, KC_S, KC_T, KC_R, KC_N, KC_P    },
  { LSFT(KC_7),        LT(_FUNC,NO_OSLH), LT(_CONTROLS,KC_J),LT(_SIGNS,KC_K),   LT(_PARENS,NO_AA),  KC_Z, KC_B, KC_C, KC_X, KC_V, KC_W, NO_MINS },
  THUMB_ROW
},

/* MOVE
 * ,-----------------------------------------------------------------------------------.
 * |      |      |      |      |      |      | Pg Up| Home |  Up  | End  |      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      | Pg Dn| Left | Down | Right|      | Shift|
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      | THome| BEnd | Del  |      | Ctrl |
 * `-----------------------------------------------------------------------------------'
 */
[_MOVE] = {
  { _______, _______,   _______,   _______,  _______,  _______,  _______, KC_HOME,    KC_UP,     KC_END,  KC_PGUP, _______ },
  { _______, _______,   _______,   _______,  _______,  _______,  _______, KC_LEFT,    KC_DOWN,   KC_RGHT, KC_PGDN, KC_LSFT },
  { _______, _______,   _______,   _______,  _______,  _______,  _______, C(KC_HOME), C(KC_END), _______, _______, KC_LCTL },
  THUMB_ROW
},

/* NUMPAD
 * ,-----------------------------------------------------------------------------------.
 * |      |      |      |      |      |      |   *  |   7  |   8  |   9  |   +  |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |   /  |   4  |   5  |   6  |   =  |   $  |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |   %  |   1  |   2  |   3  |   -  |   £  |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |   0  | Shift|      |      |      |
 * `-----------------------------------------------------------------------------------'
 */
[_NUMPAD] = {
  { _______, _______, _______ ,_______, _______, _______, NO_ASTR,  KC_7, KC_8,    KC_9,    NO_MINS, _______},
  { _______, _______, _______ ,_______, _______, _______, NO_SLSH,  KC_4, KC_5,    KC_6,    NO_PLUS, NO_DLR },
  { _______, _______, _______ ,_______, _______, _______, KC_PERC , KC_1, KC_2,    KC_3,    NO_EQL , NO_PND  },
  { _______, _______, _______ ,_______, _______, _______, _______,  KC_0, KC_LSFT, _______, _______, _______}
},

/* PARENS
 * ,-----------------------------------------------------------------------------------.
 * |      |      |      |      |      |      |      |   "  |   '  |   `´ |      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |   |  |   (  |   )  |   {  |   }  |  /   |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |   _  |   [  |   ]  |   <  |   >  |  \   |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |      |      |      |      |      |
 * `-----------------------------------------------------------------------------------'
 */
[_PARENS] = {
  {_______, _______, _______, _______, _______, _______, _______, NO_QUO2, NO_APOS, NO_ACUT, _______, _______},
  {_______, _______, _______, _______, _______, _______, NO_PIPE, NO_LPRN, NO_RPRN, NO_LCBR, NO_RCBR, NO_SLSH},
  {_______, _______, _______, _______, _______, _______, NO_UNDS, NO_LBRC, NO_RBRC, NO_LESS, NO_GRTR, NO_BSLS},
  {_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______}
},

/* SIGNS
 * ,-----------------------------------------------------------------------------------.
 * |      |      |      |      |      |      |   =  |  +   |   -  |   *  |   %  |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |   ~  |   .  |   !  |   ?  |   &  |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |   ^  |   @  |   #  |   $  |   £  |  €   |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * `-----------------------------------------------------------------------------------'
 */
[_SIGNS] = {
  {_______, _______, _______, _______, _______, _______, NO_EQL,  NO_PLUS, NO_MINS, NO_ASTR, KC_PERC, _______},
  {_______, _______, _______, _______, _______, _______, NO_TILD, KC_DOT,  KC_EXLM, NO_QUES, NO_AMPR, _______},
  {_______, _______, _______, _______, _______, _______, NO_CIRC, NO_AT,   NO_HASH, NO_DLR,  NO_PND,  NO_EURO},
  {_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______}
},


/* MOUSE
 * ,-----------------------------------------------------------------------------------.
 * |      |      | ACC-2| ACC-1| ACC-0|      |      | SW-L |  M-U | SW-R | SW-U |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      | MB-3 | MB-2 | MB-1 |      |      |  M-L |  M-D |  M-R | SW-D |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |      |      |      |      |Reset |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |      |      |      |      |      |
 * `-----------------------------------------------------------------------------------'
 */
[_MOUSE] = {
  {_______, _______, KC_ACL2, KC_ACL1, KC_ACL0, _______, _______, KC_WH_L, KC_MS_U, KC_WH_R, KC_WH_U, _______},
  {_______, _______, KC_BTN3, KC_BTN2, KC_BTN1, _______, _______, KC_MS_L, KC_MS_D, KC_MS_R, KC_WH_D, _______},
  {_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, RESET  },
  {_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______}
},

/* CONTROLS
 * ,-----------------------------------------------------------------------------------.
 * |      |      |      |      |      |      |      | Vol+ |SlackU|      |      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      | Vol- |SlackD|      |      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |VolOff|PrScr |      |      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |      |      |      |      |      |
 * `-----------------------------------------------------------------------------------'
 */
[_CONTROLS] = {
  {_______, _______, _______, _______, _______, _______, _______, KC_VOLU, SLACKUP, _______, _______, _______},
  {_______, _______, _______, _______, _______, _______, _______, KC_VOLD, SLACKDN, _______, _______, _______},
  {_______, _______, _______, _______, _______, _______, _______, KC_MUTE, KC_PSCR, _______, _______, _______},
  {_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______}
},

 /* FUNC
 * ,-----------------------------------------------------------------------------------.
 * |      |      |      |      |      |      |      |  F7  |  F8  |  F9  | F10  |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |  F4  |  F5  |  F6  | F11  |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |  F1  |  F2  |  F3  | F12  |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |      |      |      |      |      |
 * `-----------------------------------------------------------------------------------'
 */
[_FUNC] = {
  {_______, _______, _______, _______, _______, _______, _______, KC_F7,   KC_F8,   KC_F9,   KC_F10,  _______},
  {_______, _______, _______, _______, _______, _______, _______, KC_F4,   KC_F5,   KC_F6,   KC_F11,  _______},
  {_______, _______, _______, _______, _______, _______, _______, KC_F1,   KC_F2,   KC_F3,   KC_F12,  _______},
  {_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______}
},

 /* I3WS
 * ,-----------------------------------------------------------------------------------.
 * |      |      |      |      |      |      |      |  W7  |  W8  |  W9  |      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |  W4  |  W5  |  W6  |      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |  W1  |  W2  |  W3  |      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |      |      |      |      |      |
 * `-----------------------------------------------------------------------------------'
 */
[_I3WS] = {
  {_______, _______, _______, _______, _______, _______, _______, I3_WS7,  I3_WS8,  I3_WS9,  I3_WS0, _______},
  {_______, _______, _______, _______, _______, _______, _______, I3_WS4,  I3_WS5,  I3_WS6,  I3_LNCH, _______},
  {_______, _______, _______, _______, _______, _______, _______, I3_WS1,  I3_WS2,  I3_WS3,  I3_TERM, _______},
  {_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______}
},

 /* I3MV
 * ,-----------------------------------------------------------------------------------.
 * |      |      |      |      |      |      |      |      |  Up  |      |      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      | Lef  | Down | Right|      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |      |      |      |      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |      |      |      |      |      |
 * `-----------------------------------------------------------------------------------'
 */
[_I3MV] = {
  {_______, _______, _______, _______, _______, _______, _______, _______, I3_FUP,  _______, _______, _______},
  {_______, _______, _______, _______, _______, _______, _______, I3_FLFT, I3_FDWN, I3_FRGT, _______, _______},
  {_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______},
  {_______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______}

}



};

